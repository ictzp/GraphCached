...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    if (nbytes < 2) {
        fprintf(stderr, "allocation bytes is less than 2\n");
        return 0;
    }
    ...
    if (id == 0) {
        fprintf(stderr, "allocation slab id == 0 ntotal == %ld\n", ntotal);
        return 0;
    }
    ...
    if (ntotal > settings.slab_chunk_size_max) {
        ...
        int htotal = nkey + 1 + sizeof(item) + sizeof(item_chunk);
	if (settings.use_cas) {
...
void do_item_unlink_nolock(item *it, const uint32_t hv) {
    MEMCACHED_ITEM_UNLINK(ITEM_key(it), it->nkey, it->nbytes);
    //printf("do_item_unlink_nolock\n");
    if ((it->it_flags & ITEM_LINKED) != 0) {
        //printf("do_item_unlink_nolock: item_linked\n");
        it->it_flags &= ~ITEM_LINKED;
//        STATS_LOCK();
//        stats_state.curr_bytes -= ITEM_ntotal(it);
//        stats_state.curr_items -= 1;
...
//            pthread_mutex_unlock(&c->thread->stats.mutex);
            ...
            if (do_update) {
                 ...
                 */
		// We dont support segmented lru yet.
                if (settings.lru_segmented) {
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        ...
        if (search->nbytes == 0 && search->nkey == 0 && search->it_flags == 1) {
            ...
            if (flags & LRU_PULL_CRAWL_BLOCKS) {
                pthread_mutex_unlock(&lru_locks[id]);
                printf("lru_pull_crawl_blocks return 0\n");
		return 0;
            }
	    printf("search->nbytes == 0 & search->nkey == 0\n");
            tries++;
            continue;
        }
	   ...
	   // printf("didn't get lock\n");
        ...
        switch (cur_lru) {
		    ...
		    //printf("cold lru evict\n");
                    if (settings.evict_to_free == 0) {
                        /* Don't think we need a counter for this. It'll OOM.  */
                        break;
                    }
		    //printf("settings.evit_to_free == 0\nsearch=%p\n", search);
		    ...
		    //printf("3\n");
                    ...
                    //printf("before do_item_unlink_nolock\n");
		    int clsid = search->slabs_clsid;
		    do_item_unlink_nolock(search, hv);
                    removed++;
